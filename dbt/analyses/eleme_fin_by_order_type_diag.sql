-- =============================================================================
-- 饿了么：按 订单类型 诊断 — SUM(结算金额) vs SUM(订单应收)
-- =============================================================================
...
by_type AS (
  SELECT
    COALESCE(NULLIF(BTRIM(REPLACE(f."订单类型", E'\t', '')), ''), '(空)') AS 订单类型,
    BOOL_OR(BTRIM(REPLACE(f."订单类型", E'\t', '')) LIKE '%赔偿%') AS 类型含赔偿字样,
    COUNT(*)::bigint AS 行数,
    SUM(
  ...
  INNER JOIN matched_order m
    ON m.order_no = NULLIF(TRIM(REPLACE(f."订单号", E'\t', '')), '')
  WHERE f.dt::date >= p.d - 120
  GROUP BY 1
)
