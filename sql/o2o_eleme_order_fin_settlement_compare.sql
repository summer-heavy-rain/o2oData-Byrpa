...
--   **财务侧**：同一 **订单号** 在窗内对 **订单应收_col_13** 做 **SUM**（多行轧差如 36.99+(−41.29)）。
--   禁止只用 DISTINCT ON 取一行。
-- 诊断：另输出导出 **原始** 商户应收合计（含全退非零），与规则口径并列，便于排查导出脏数据。
--
-- 逻辑 A（财务取数逻辑 / 结算口径）：**SUM(订单应收_col_13)** GROUP BY 订单号（窗 dt>=D-120）
...
--   + 骑手小费(其他支出_col_83) + 优惠合计(其他支出_col_81)
-- 等价于：净营业额合计 + 服务费用合计（行级恒等，2026-03-28 验证）
--
-- 完整「结算金额列」口径（与平台 SUM(结算金额_col_11) 一致）：
--   订单类型_col_9 **含「赔偿」字样** 的行：`订单应收_col_13` 恒为 0，**仅 `结算金额_col_11` 有值**；
--   其余行：`结算金额_col_11` = `订单应收_col_13`（与 11 项公式还原一致）。
--   故：**按单汇总** `SUM(结算金额_col_11) = SUM(订单应收_col_13) + SUM(结算金额_col_11 WHERE 订单类型 LIKE '%赔偿%')`。
...
ele_fin_pick AS (
  SELECT
    ...
    SUM(
      CASE WHEN BTRIM(REPLACE(f."结算金额_col_11", E'\t', '')) ~ '^-?[0-9]+(\.[0-9]*)?$'
           AND BTRIM(REPLACE(f."订单类型_col_9", E'\t', '')) LIKE '%赔偿%'
           THEN BTRIM(REPLACE(f."结算金额_col_11", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS fin_peichang_jiesuan_col11,
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_17", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_17", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS spje,
    SUM(
    ...
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_55", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_55", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS shishou_yongjin,
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_59", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_59", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS shishou_zhifu,
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_20", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_20", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS huodong_butie,
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_21", E'\t', '')) ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_21", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS daijinquan_butie,
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_28", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家收入_净营业额合计_col_28", E'\t', ''))::numeric ELSE 0::numeric END
    ...
    SUM(
      CASE WHEN BTRIM(REPLACE(f."其他支出_col_83", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."其他支出_col_83", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS qishou_xiaofei,  -- 骑手小费（其他支出，非服务费用）
    SUM(
      CASE WHEN BTRIM(REPLACE(f."其他支出_col_81", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."其他支出_col_81", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS youhui_heji  -- 优惠金额合计（其他支出，非服务费用）
  FROM ods_rpa_eleme_fin_sales_detail f
  INNER JOIN matched_order k
    ON k.order_no = NULLIF(TRIM(REPLACE(f."订单号_col_6", E'\t', '')), '')
  CROSS JOIN params p
  WHERE f.dt::date >= p.d - 120
  GROUP BY 1
),
...
joined AS (
  SELECT
    ...
    (
      ...
      + COALESCE(p.qishou_xiaofei, 0)
      + COALESCE(p.youhui_heji, 0)
    ) AS formula_rebuilt
...
--   → 按 订单类型_col_9 汇总 sum(结算金额_col_11) − sum(订单应收_col_13)，排查除赔偿外是否有新类型轧差。
-- -----------------------------------------------------------------------------
