...
-- =============================================================================
-- 逻辑一（现行聚合）：美团=财务「商家应收款」；饿了么=财务「订单应收」（订单应收_col_13）
-- 逻辑二（分项重算）：
...
ele_fin_pick AS (
  SELECT
    ...
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_60", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_60", E'\t', ''))::numeric ELSE 0::numeric END
    ...
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家收入资金来源_col_41", E'\t', '')) ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家收入资金来源_col_41", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS daijinquan_butie,
    SUM(
    ...
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_74", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_74", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS qishou_xiaofei,
    SUM(
      CASE WHEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_75", E'\t', ''))
                ~ '^-?[0-9]+(\.[0-9]*)?$'
           THEN BTRIM(REPLACE(f."商家支出_服务费用合计_col_75", E'\t', ''))::numeric ELSE 0::numeric END
    ) AS youhui_heji
  FROM ods_rpa_eleme_fin_sales_detail f
...
ele_agg AS (
  SELECT
    ...
    COALESCE(SUM(
      ...
      + COALESCE(qishou_xiaofei, 0)
      + COALESCE(youhui_heji, 0)
